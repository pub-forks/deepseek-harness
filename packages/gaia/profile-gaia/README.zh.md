...
Gaia 使用指向 `gaia.patch.yml` 的 `--patch` 启动内置 `web` 配置。覆盖层将主机绑定到本机回环地址，打印带令牌的 URL 而不打开浏览器，关闭 HMR 和产品遥测，启用使用已存 OAuth 凭据的 `openai-codex` 路由，并挂载 Gaia 控制桥。基础组合包已经将 Session JSONL 存在 `$DSH_HOME/sessions` 下。
